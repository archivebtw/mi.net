-- mi.net Auth v2 database verification/fix
...
begin
  if input_username is null then return false; end if;

  -- ASCII / English handles only.
  if input_username !~ '^[A-Za-z0-9_.-]{3,24}$' then return false; end if;
  if input_username ~ '^[._-]' or input_username ~ '[._-]$' then return false; end if;
  if input_username ~ '[._-]{2,}' then return false; end if;

  normalized := lower(regexp_replace(input_username, '[_.-]+', '', 'g'));

  foreach banned in array banned_words loop
    if strpos(normalized, banned) > 0 then return false; end if;
  end loop;
...
-- Verification. Both should return true / one row after setup.
select public.is_username_allowed('archivebtw') as archivebtw_is_allowed;
