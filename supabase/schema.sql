-- mi.net Supabase Auth + public profile schema
...
declare
  ...
  banned_words text[] := array[
    ...
    'сука','сучк','мудак','долбоеб','долбоёб','пидор','пидар','педик','шлюх','нацист'
  ];
begin
  ...
  foreach banned in array banned_words loop
    if strpos(normalized, lower(banned)) > 0 then
      return false;
    end if;
  end loop;
